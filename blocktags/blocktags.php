	...
	*/
	function hookLeftColumn($params)
	{
		if (!$this->isCached('blocktags.tpl', $this->getCacheId('blocktags')))
		{
			...
			{
			    $cat_id = (int)Tools::getValue('id_category');
				$products = Product::getProducts(1, 0, 0, 'id_product', 'ASC',  $cat_id, TRUE);
				$tags1 = array();
				foreach($products as $product) {
					$tags_array = Tag::getProductTags($product['id_product']);
					foreach($tags_array[1] as $c_tag) {
						if(empty($c_tag)) continue;
						$tags1[$c_tag][] = $c_tag;
					}
				}
				...
				foreach($tags1 as $k => $v) {
					$tags[] = array("name" => $k, "times" => count($v));
				}
